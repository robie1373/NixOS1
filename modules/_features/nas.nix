...
let
  ...
  nasUmount = pkgs.writeShellApplication {
    name = "nas-umount";
    runtimeInputs = [ ];
    text = ''
      MOUNT="''${1:-/mnt/nas}"
...
in
{
  # cifs-utils provides mount.cifs; nas-mount/nas-umount are user-facing scripts
  environment.systemPackages = [ pkgs.cifs-utils nasMount nasUmount ];
