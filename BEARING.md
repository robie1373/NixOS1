...
> 1. bearing-status + bearing-log shell scripts (see below — new, quick)
> 2. Host config for "five" (see below)
> 3. diagnose and fix the nixos mcp error.
> 4. Bearing refactor

- [ ] **[pri 2] New host config — "five" (192.168.7.137).** Build a full NixOS host configuration for a graphical desktop machine.

  **Host facts:**
  - Hostname: `five`
  - Current IP: 192.168.7.137 (home VLAN, 192.168.7.x)
  - Has an NVIDIA GPU (used for Whisper transcription — was already running a quick/dirty NixOS install for this purpose)
  - Currently dual-boot Windows / NixOS (Windows retained for Virpil firmware config)
  - Will be a proper declarative config replacing the quick/dirty install

  **Purpose — configure all of the following:**
  1. **KDE Plasma desktop** (Wayland). This machine is a gaming desktop, Plasma is the right call over niri/tiling. Full desktop environment.
  2. **NVIDIA drivers** — proprietary, with CUDA support (required for Ollama GPU inference and Whisper GPU acceleration).
  3. **Ollama** — local LLM server, GPU-accelerated. Should be a systemd service accessible on the local network (at minimum from flipper on the same subnet). Use `services.ollama` if available in nixpkgs stable, otherwise package it manually.
  4. **openai-whisper** — for audio transcription jobs. Available in nixpkgs. Should be on PATH for the user. GPU acceleration via CUDA.
  5. **Steam + Proton** — `programs.steam.enable = true`. Include `hardware.steam-hardware.enable = true` for controller support. Proton/Steam Play for Windows games.
  6. **Virpil HOTAS udev rules** — the sticks need HIDRAW access for games to see them correctly. Reference: https://github.com/LunaBaloona/Virpil_devices_on_Linux for the correct udev rules and any kernel parameters needed. The devices are USB HID — the user (robie) must be in the appropriate group.
  7. **Tailscale** — same as all other hosts. Join the tailnet.

  **Stable channel:**
  This host should track **nixos-stable** (currently 25.11), not nixos-unstable. The nixos-config flake likely uses unstable as the primary input. Add a `nixpkgs-stable` input and wire `five` to use it. Check `flake.nix` and existing host patterns before deciding the cleanest approach — the goal is stable for gaming reliability without breaking the existing unstable hosts. If the flake already has a stable input, use it.

  **What to produce:**
  - `hosts/five/configuration.nix` — host-level config (hardware, services, system packages)
  - `hosts/five/hardware-configuration.nix` — either generate from the existing install or write a reasonable placeholder with a clear note that it must be regenerated on the actual machine via `nixos-generate-config`
  - Wire `five` into `flake.nix` `nixosConfigurations` against the stable nixpkgs input
  - Add `five` to `docs/homeLab-state-map.md` or equivalent host registry

  **Don't do:** Don't run nixos-anywhere or deploy anything. This is config authoring only. Note what needs to happen at deploy time (regenerate hardware-configuration.nix, run nixos-rebuild switch, join tailnet).

  Delegated 2026-04-17.

- [x] **[NEXT — pri 1] Add bearing-status and bearing-log shell scripts to bearing.nix.** Two small offline-capable scripts to keep the bearing functional when Claude/tokens are unavailable. Add as `pkgs.writeShellScriptBin` derivations in `modules/home/bearing.nix`, added to `home.packages`.

  **`bearing-status`** — reads local files only, no AI needed. Outputs a compact status card to stdout:
  - Header: day of week + date
  - Obligations: items from `~/work/OBLIGATIONS.md` where Date is today or tomorrow (date-grep)
  - Todos: active rows from the Todos table in OBLIGATIONS.md (any row with a due date)
  - Recurring: items from Recurring table that are due (compare Last done + frequency to today)
  - Korean: last session date, current streak (consecutive days back from today), total unique days — computed from `~/languages/study-robie.log` using awk/date
  - Next up: lines from `~/work/DELEGATIONS.md` containing `**next**` or `[pri 1]`

  **`bearing-log`** — opens today's log for editing:
  - Checks if `~/work/log/YYYY-MM-DD.md` exists; if not, creates it with the standard template (Morning/Activities/Evening gap/Notes sections)
  - Opens it in `$EDITOR` (fallback: `nano`)

  **Korean streak logic:** iterate backwards from today checking `study-robie.log` for each date until a gap is found. Use `date -d` (GNU) with a fallback for BSD date (`date -j -f`). NixOS uses GNU coreutils so `date -d` is fine.

  **Template for bearing-log** (create if file missing):
  ```
  # YYYY-MM-DD
  ...
  Delegated 2026-04-17, priority: do first.

- [x] **[pri 1] Add Korean language support.** Add Korean input method support to the NixOS config (likely fcitx5 or ibus with Korean IME). Goal: enable Korean character input system-wide, in preparation for Hangul keyboard stickers and switching from romanized input. Delegated 2026-03-31 — complete 2026-04-01
...
- [ ] **Tailscale watchdog — auto-reconnect systemd timer.** Tailscale devices are falling off the tailnet and not reconnecting automatically when routing is restored. Implement a systemd timer (every 15 min) on all NixOS hosts that checks `tailscale status --json` and runs `tailscale up` if BackendState is not "Running". Use the existing non-interactive auth key module in nixos-config as the key source. Deploy as a shared NixOS module to all tailnet hosts. Also audit the tailnet admin console for expired or expiring keys — a watchdog won't fix an expired key. — Delegated 2026-04-14
...
**bearing-status and bearing-log complete (2026-04-17).** Both scripts added to `modules/_home/bearing.nix` and verified working. `bearing-status` outputs a compact offline card: obligations (today/tomorrow), todos, recurring (with frequency/last-done arithmetic), Korean streak (25 days, 25 total unique days as of today), and next-up items from DELEGATIONS.md. `bearing-log` opens or creates `~/work/log/YYYY-MM-DD.md` with the standard template. Available on PATH after `nh os switch`.
